...
class TyrosineGraphScreen extends StatefulWidget {
  ...
  const TyrosineGraphScreen({super.key, required this.patientRecord});
...
class _TyrosineGraphScreenState extends State<TyrosineGraphScreen> {
  ...
  Widget _buildDataTable(List<TyrosineRecord> records) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        const Text('Kan Tirozin Düzeyi Vizit Verileri (Düzenlemek İçin Tıklayın)', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
        const SizedBox(height: 8),
        DataTable(
          columnSpacing: 20,
          horizontalMargin: 12,
